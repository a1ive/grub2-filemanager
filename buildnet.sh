...
cd build
#find ./boot | cpio -o -H newc | gzip -9 > ./netfm.loop
cd ..
#rm -r build/boot
cp arch/legacy/MAP build/boot/grubfm/
cp -r arch/legacy/ntboot/* build/boot/grubfm/
#cp arch/legacy/wimboot build/boot/grubfm/
#cp arch/legacy/vbootldr build/boot/grubfm/
cp arch/legacy/install.gz build/boot/grubfm/
cd build
rm -r ../tftpboot
mkdir ../tftpboot
